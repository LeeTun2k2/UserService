...
To update the database use this command from the Web project folder (replace `UserService` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../UserService.Infrastructure/UserService.Infrastructure.csproj -s UserService.Web.csproj
```
